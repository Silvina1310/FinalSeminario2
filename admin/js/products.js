...
$(document).ready(function(){
		...
		$.ajax({
			...
			success : function(response){
				...
				if (resp.status == 202) {
					...
					var catSelectHTML = '<option value="">Elija Categoría</option>';
					...
					var brandSelectHTML = '<option value="">Elija Rango de Precios</option>';
	...
	$(document.body).on('click', '.delete-product', function(){
		...
		if (confirm("¿Estás segura/o de eliminar este artículo? ?")) {
			$.ajax({
				url : '../admin/classes/Products.php',
				method : 'POST',
